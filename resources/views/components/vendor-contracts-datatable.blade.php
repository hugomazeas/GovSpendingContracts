@props(['ajaxUrl', 'vendorName'])

<div class="card mt-8">
    <div class="flex items-center mb-6 pb-4 border-b border-gray-200">
        <i class="fas fa-file-contract text-2xl text-indigo-500 mr-3"></i>
        <h3 class="text-xl font-semibold text-gray-800">{{ __('app.vendor_contracts') }}</h3>
    </div>

    <div class="overflow-x-auto">
        <table id="vendor-contracts-table" class="min-w-full">
            <thead>
    ...
    <div class="mt-4 p-4 bg-blue-50 rounded-lg">
        <div class="flex items-start space-x-3">
            ...
            <div class="text-sm text-blue-700">
                ...
                <p>{{ __('app.vendor_contracts_description', ['vendor' => $vendorName]) }}</p>
            </div>
        </div>
    </div>
</div>
...
$(document).ready(function() {
    window.vendorContractsTable = $('#vendor-contracts-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            url: '{{ $ajaxUrl }}',
